...
def make_mesh_object(name: str, files: List[str], scale: float, obj_name, way):
    ...
    if obj_name=="stapler" and way == "use":
        geoms = [
            f'<geom type="mesh" class="robot0:DC_obj" mesh="{name}-{idx}" pos="0 0 0.015" euler="-1.571 0 0"/>'
            for idx in range(len(files))
        ]        
        geoms.append(f'<geom type="mesh" mesh="{name}" class="robot0:D_Vizual" euler="-1.571 0 0" pos="0 0 0.015" rgba = "0.5 0.5 0.5 0.8"/>') 
    else:    
        geoms = [
...
class Controller():
    ...
    def control(self, data, niter):
        sensor_data = []

        for i in range(1000):
            self.apply_torque(self.xs_joint_id, self.xs_ac_id, data[0], 0)
            self.apply_torque(self.ys_joint_id, self.ys_ac_id, data[1], 0)
            self.apply_torque(self.zs_joint_id, self.zs_ac_id, 0.1, 0, kp=10000)        
            self.apply_torque(self.xr_joint_id, self.xr_ac_id, data[3], 0)
            self.apply_torque(self.yr_joint_id, self.yr_ac_id, data[4], 0)
            self.apply_torque(self.zr_joint_id, self.zr_ac_id, data[5], 0)
            self.apply_torque(self.th1_joint_id,self.th1_ac_id, data[11] / niter * i, 0)
            self.apply_torque(self.ff_joint_id, self.ff_ac_id, 0, 0)
            self.apply_torque(self.mf_joint_id, self.mf_ac_id, 0, 0)
            self.apply_torque(self.rf_joint_id, self.rf_ac_id, 0, 0)
            self.apply_torque(self.lf_joint_id, self.lf_ac_id, 0, 0)
            self.apply_torque(self.th2_joint_id,self.th2_ac_id, 0, 0)
        ...
        for i in range(niter + 1):
            self.apply_torque(self.xs_joint_id, self.xs_ac_id, (data[0]), 0)      
            self.apply_torque(self.ys_joint_id, self.ys_ac_id, (data[1]), 0)
            self.apply_torque(self.zs_joint_id, self.zs_ac_id, 0.1 + (data[2] - 0.1) / niter * i, 0, kp=10000)
            self.apply_torque(self.xr_joint_id, self.xr_ac_id, data[3], 0)
            self.apply_torque(self.yr_joint_id, self.yr_ac_id, data[4], 0, kp=1000)
            self.apply_torque(self.zr_joint_id, self.zr_ac_id, data[5], 0)
            self.apply_torque(self.ff_joint_id, self.ff_ac_id, data[6] / niter * i, 0)
            self.apply_torque(self.mf_joint_id, self.mf_ac_id, data[7] / niter * i, 0)
            self.apply_torque(self.rf_joint_id, self.rf_ac_id, data[8] / niter * i, 0)
            self.apply_torque(self.lf_joint_id, self.lf_ac_id, data[9] / niter * i, 0)
            self.apply_torque(self.th2_joint_id, self.th2_ac_id, data[10] / niter * i, 0)
            self.apply_torque(self.th1_joint_id, self.th1_ac_id, data[11], 0, kp=20)
            self.sim.step()
            self.viewer.render()
        ...
        for i in range(1000):
            self.apply_torque(self.xs_joint_id, self.xs_ac_id, data[0], 0)      
            self.apply_torque(self.ys_joint_id, self.ys_ac_id, data[1], 0)
            self.apply_torque(self.zs_joint_id, self.zs_ac_id, data[2], 0)
            self.apply_torque(self.xr_joint_id, self.xr_ac_id, data[3], 0)
            self.apply_torque(self.yr_joint_id, self.yr_ac_id, data[4], 0)
            self.apply_torque(self.zr_joint_id, self.zr_ac_id, data[5], 0)
            self.apply_torque(self.ff_joint_id, self.ff_ac_id, data[6], 0, fd_torque= 0.1)
            self.apply_torque(self.mf_joint_id, self.mf_ac_id, data[7], 0, fd_torque= 0.1)
            self.apply_torque(self.rf_joint_id, self.rf_ac_id, data[8], 0, fd_torque= 0.1)
            self.apply_torque(self.lf_joint_id, self.lf_ac_id, data[9], 0, fd_torque= 0.1)
            self.apply_torque(self.th2_joint_id, self.th2_ac_id, data[10], 0)
            self.apply_torque(self.th1_joint_id, self.th1_ac_id, data[11], 0, fd_torque= 0.1)
            sensor_data.append(self.sim.data.sensordata.copy())
            self.sim.step()
            self.viewer.render()
        ...
        for i in range(5000):
            self.apply_torque(self.xs_joint_id, self.xs_ac_id, data[0], 0)
            self.apply_torque(self.ys_joint_id, self.ys_ac_id, data[1], 0)
            self.apply_torque(self.zs_joint_id, self.zs_ac_id, data[2] + 0.0001 * i, 0)
            self.apply_torque(self.xr_joint_id, self.xr_ac_id, data[3], 0)
            self.apply_torque(self.yr_joint_id, self.yr_ac_id, data[4], 0)
            self.apply_torque(self.zr_joint_id, self.zr_ac_id, data[5], 0)
            self.apply_torque(self.ff_joint_id, self.ff_ac_id, data[6] + 0.1, 0, fd_torque= 0.2)
            self.apply_torque(self.mf_joint_id, self.mf_ac_id, data[7] + 0.1, 0, fd_torque= 0.2)
            self.apply_torque(self.rf_joint_id, self.rf_ac_id, data[8] + 0.1, 0, fd_torque= 0.2)
            self.apply_torque(self.lf_joint_id, self.lf_ac_id, data[9] + 0.1, 0, fd_torque= 0.2)
            self.apply_torque(self.th2_joint_id, self.th2_ac_id, data[10], 0)
            self.apply_torque(self.th1_joint_id, self.th1_ac_id, data[11] + 0.1, 0, fd_torque= 0.2)
            sensor_data.append(self.sim.data.sensordata.copy())
            self.sim.step()
            self.viewer.render()        
...
def main():
    ...
    ini = []
    ini.append(cfg['ini'][str(objs[indices[0]])][way[i]])
    x0 = np.array(ini)
    ...
    tree = ET.parse('E://zju//desktop_organization//functional grasp//frame_work//assets//basic.xml')

    xmls = []

    object_xml = make_mesh_object(f"object", candidates[indices[0]], scale=0.1, obj_name = str(objs[indices[0]]), way = way[i])
