...
def main():
    data = np.load("functional grasp/data/stalper_use.npy")
    model = load_model_from_path("E:/zju/desktop_organization/functional grasp/assets/stalper_use.xml")
    sim = MjSim(model)
    # viewer set up
    viewer = MjViewer(sim)
    hand = Controller(sim)
    ...
    for i in range(1000):
        ...
        hand.apply_torque(hand.zs_joint_id, hand.zs_ac_id, data[2] + 0.1, 0, kp=10000)

        hand.apply_torque(hand.xr_joint_id, hand.xr_ac_id, data[3], 0)
        hand.apply_torque(hand.yr_joint_id, hand.yr_ac_id, data[4], 0, kp=1000)
    ...
    for i in range(niter + 1):
        hand.apply_torque(hand.xs_joint_id, hand.xs_ac_id, data[0] + (data[0] - data[0]) / niter * i, 0)      
        hand.apply_torque(hand.ys_joint_id, hand.ys_ac_id, data[1] + (data[1] - data[1]) / niter * i, 0)
        hand.apply_torque(hand.zs_joint_id, hand.zs_ac_id, data[2] + 0.1 + (data[2] - data[2] - 0.1) / niter * i, 0, kp=10000)
        hand.apply_torque(hand.xr_joint_id, hand.xr_ac_id, data[3] + (data[3] - data[3]) / niter * i, 0)
        hand.apply_torque(hand.yr_joint_id, hand.yr_ac_id, data[4] + (data[4] - data[4])  / niter * i, 0, kp=1000)
        hand.apply_torque(hand.zr_joint_id, hand.zr_ac_id, data[5] + (data[5] - data[5])  / niter * i, 0)
    ...
    for i in range(5000):
        ...
        hand.apply_torque(hand.yr_joint_id, hand.yr_ac_id, data[4], 0)
        hand.apply_torque(hand.zr_joint_id, hand.zr_ac_id, data[5], 0)
        hand.apply_torque(hand.ff_joint_id, hand.ff_ac_id, data[6], 0, fd_torque= 0.3)
        hand.apply_torque(hand.mf_joint_id, hand.mf_ac_id, data[7], 0, fd_torque= 0.3)
        hand.apply_torque(hand.rf_joint_id, hand.rf_ac_id, data[8], 0, fd_torque= 0.3)
        hand.apply_torque(hand.lf_joint_id, hand.lf_ac_id, data[9], 0, fd_torque= 0.3)
        hand.apply_torque(hand.th2_joint_id, hand.th2_ac_id, data[10], 0)
        hand.apply_torque(hand.th1_joint_id, hand.th1_ac_id, data[11], 0, fd_torque= 0.3)
        sensor_data.append(sim.data.sensordata.copy())
        sim.step()
        viewer.render()
